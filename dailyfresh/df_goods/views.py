...
def index(request):
    ...
    context={'title':'首页', 'guest_cart':1, 'page_name':0,
...
def list(request,tid,pindex,sort):
    ...
    if sort=='1':  # 默认-最新
        goods_list=GoodsInfo.object.filter(gtype_id=int(tid)).ordre_by('-id')
    elif sort=='2':  # 价格
        goods_list=GoodsInfo.object.filter(gtype_id=int(tid)).order_by('gprice')
    elif sort=='3':  # 人气-点击量
        goods_list = GoodsInfo.object.filter(gtype_id=int(tid)).order_by('-gclick')
